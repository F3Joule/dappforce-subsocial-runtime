...
impl<T: Trait> Module<T> {
  ...
  */
  ...
  pub fn share_post(account: T::AccountId, on_behalf: Option<T::SpaceId>, original_post_id: T::PostId, shared_post_id: T::PostId) -> Result {
    let mut original_post = Self::post_by_id(original_post_id).ok_or(MSG_ORIGINAL_POST_NOT_FOUND)?;

    if let PostExtension::SharedPost(_) = original_post.extension {
      return Err(MSG_CANNOT_SHARE_SHARED_POST)
    }
    ...
    match original_post.extension {
      PostExtension::RegularPost => {
        <PostSharesByAccount<T>>::insert((account, original_post_id), shares_by_account); // TODO Maybe use mutate instead?
      ...
      PostExtension::Comment(_) => {
        Self::deposit_event(RawEvent::CommentShared(spaced_account, original_post_id));
      },
      PostExtension::SharedPost(_) => (),
    }
